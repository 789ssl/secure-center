...
(function () {
	...
	if (!global.performance) {
		global.performance = {
			now: function now() {
				...
				var sec = _process$hrtime2[0];
				var nsec = _process$hrtime2[1];
	...
	global.Go = function () {
		...
		_createClass(_class, [{
			key: "run",
			value: async function run(instance) {
				var _this2 = this;
				...
				var strPtr = function strPtr(str) {
					var ptr = offset;
				...
				var keys = Object.keys(this.env).sort();
				keys.forEach(function (key) {
					argvPtrs.push(strPtr(key + "=" + _this2.env[key]));
				});
				argvPtrs.push(0);
